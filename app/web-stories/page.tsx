...
export const metadata: Metadata = {
  title: 'Web Stories | StudyPoint',
  description: 'Watch our latest web stories and vertical videos.',
};
...
export default async function WebStoriesPage() {
  await connectDB();
  const reels = await Reel.find({ category: 'web-stories' }).sort({ createdAt: -1 }).lean();

  return (
    <div className="section-padding" style={{ backgroundColor: 'var(--gray-50)', minHeight: '100vh' }}>
      <div className="container">
        <h1 className="article-title" style={{ textAlign: 'center', marginBottom: '3rem' }}>Web Stories</h1>
        
        {reels.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem', color: 'var(--gray-500)', fontSize: '1.1rem' }}>
            <p>No web stories available at the moment. Check back later!</p>
          </div>
        ) : (
